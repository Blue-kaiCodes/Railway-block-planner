// Curated Indian Railways Station Directory
...
function enrichStation(s) {
  ...
  const clone = { ...s };
  if (!clone.division) {
    if (clone.city === "Kolkata" && clone.code === "HWH") clone.division = "Howrah (HWH)";
    else if (clone.city === "Kolkata") clone.division = "Sealdah (SDAH)";
    else if (clone.district && clone.district.includes("Bardhaman")) clone.division = clone.code === "ASN" ? "Asansol (ASN)" : "Howrah (HWH)";
    else if (clone.code === "DHN") clone.division = "Dhanbad (DHN)";
    else if (clone.code === "DDU" || clone.district === "Chandauli") clone.division = "Pt. Deen Dayal Upadhyaya (DDU)";
    else if (clone.code === "GAYA") clone.division = "Pt. Deen Dayal Upadhyaya (DDU)";
    else if (clone.district === "Patna") clone.division = "Danapur (DNR)";
    else if (clone.code === "PRYJ" || clone.code === "CNB") clone.division = "Prayagraj (PRYJ)";
    else if (clone.state === "Delhi") clone.division = "Delhi (DLI)";
    else if (clone.district) clone.division = `${clone.district} Division`;
    else clone.division = `${clone.zone || 'IR'} Division`;
  }
  if (!clone.category) {
    if (clone.platforms >= 10) clone.category = "NSG-1 Premier";
    else if (clone.platforms >= 6) clone.category = "NSG-2 Major";
    else if (clone.platforms >= 4) clone.category = "NSG-3 Medium";
    else clone.category = "NSG-4 Sub-Divisional";
  }
  if (clone.electrified === undefined) {
    clone.electrified = true;
  }
  return clone;
}
...
for (let i = 0; i < STATIONS_DB.length; i++) {
  STATIONS_DB[i] = enrichStation(STATIONS_DB[i]);
}

export function searchStations(query = "", state = null, district = null, limit = 15) {
  const q = (query || "").trim().toLowerCase();
  const results = [];

  for (const s of STATIONS_DB) {
    if (state && state !== "ALL" && s.state.toLowerCase() !== state.toLowerCase()) {
      continue;
    }
    if (district && district !== "ALL" && s.district.toLowerCase() !== district.toLowerCase()) {
      continue;
    }
    ...
    if (!q) {
      ...
      const codeMatch = s.code.toLowerCase().startsWith(q) || s.code.toLowerCase().includes(q);
      const nameMatch = s.name.toLowerCase().includes(q);
      const cityMatch = s.city.toLowerCase().includes(q);
      const distMatch = s.district.toLowerCase().includes(q);
      const stateMatch = s.state.toLowerCase().includes(q);

      if (codeMatch || nameMatch || cityMatch || distMatch || stateMatch) {
        const score = s.code.toLowerCase() === q ? 10 : (codeMatch ? 5 : 1);
        results.push({ score, station: s });
      }
    }
  }
  ...
  if (q) {
    ...
    return results.slice(0, limit).map(item => item.station);
  }
...
export function getStationByCode(code) {
  ...
  return STATIONS_DB.find(s => s.code === c) || null;
}

export function getAllStates() {
  return Array.from(new Set(STATIONS_DB.map(s => s.state))).sort();
}
...
export function getStationsByDistrict(state, district) {
  return STATIONS_DB.filter(
    s => s.state.toLowerCase() === (state || "").toLowerCase() &&
         s.district.toLowerCase() === (district || "").toLowerCase()
  );
}

export function getGeographicalTree() {
  const tree = {};
  for (const s of STATIONS_DB) {
    const st = s.state;
    const dst = s.district;
    if (!tree[st]) tree[st] = {};
    if (!tree[st][dst]) tree[st][dst] = [];
    tree[st][dst].push({
      code: s.code,
      name: s.name,
      city: s.city,
